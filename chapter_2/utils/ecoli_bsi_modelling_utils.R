...
# Modelling and analytical functions for baseline_models.Rmd
# author: Chang Ho Yoon

# winsorisation / truncation of continuous variables
...
factorise_categorical <- function(dataframe, factorise_all = TRUE, list_of_categorical_variables, factorise_multilevel = TRUE){
  ...
  if (factorise_multilevel == TRUE){
    ...
    df$Group <- factor(df$Group, levels=c('Active coamoxiclav only', 'Active coamoxiclav + aminoglycoside', 'Active coamoxiclav + other', 'Inactive coamoxiclav only', 'Inactive coamoxiclav + active aminoglycoside', 'Inactive coamoxiclav + active other'))
...
# create dataframe for survival modelling
make_survival_df <- function(pre_analysis_df, winsorise_continuous = FALSE, sym_wins_vars, sym_limits = c(0.025, 0.975), asym_upper_vars, asym_upper_limits = c(0, 0.95), asym_lower_vars, asym_lower_limits = c(0.05, 1)){
  ...
  edited_df <- pre_analysis_df %>%
    select(ClusterID, SpellID, Group, collectiondatetime,
           
           # categorical variables:
           LinkedSex, Ethnicity, Specialty, AdmissionTimeOfDay, AdmissionDayOfWeek, OutOfHoursAdmission, PriorHospitalisation, AVPU, SupplementaryOxygen, diabetes, dialysis, immunosuppression, palliative, CommunityOnset, PriorEcoliBacteraemiaEpisode, Polymicrobial,
           ...
           InactiveCoamoxiclav, InactiveAminoglycoside, InactiveOther, categorical_mic_coamoxiclav, HighRbreakpoints, EUCASTbreakpoints, GivenBaselineAminoglycoside,
           ### source
           urinary_source, chest_source, abdominal_source, other_source, surgical_source, Multi_source,
           
           
           # continuous variables:
           log2_mic_coamoxiclav, log2_mic_aminoglycoside, Weight, Height, BMI, SBP, DBP, HeartRate, RespiratoryRate, OxygenSaturation, Temperature, albumin, alkaline_phosphatase, alt, aptt, ast, bilirubin, calcium_adjusted, creatinine, egfr, eosinophils, haematocrit, haemoglobin, immature_granulocytes, 
           lymphocytes, magnesium, mch, mcv, monocytes, neutrophils, phosphate, platelets, potassium, pt, sodium, urea, white_cells, 
...
# forward selection using multiple fractional polynomials (needs mfp package)
